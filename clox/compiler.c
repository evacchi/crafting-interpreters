...
static void endScope() {
  ...
  // remove locals when leaving scope
...
// so we know the variable isn’t in the current compiler
static int resolveLocal(Compiler* compiler, Token* name) {
  // We walk the array backwards so that we find 
  // the *last* declared variable with that identifier
...
static void markInitialized() {
  // at scopeDepth == 0 it is a global, thus return
  if (current->scopeDepth == 0) return;
  current->locals[current->localCount - 1].depth =
      current->scopeDepth;
}
...
static void classDeclaration() {
  ...
  if (match(TOKEN_LESS)) {
    ...
    // create scope so 'super' variable don't collide
    // in the chain of superclasses
    beginScope();
...
static void ifStatement() {
  ...
  // here we do not know yet how far to jump
  ...
  // after the statement we know: 
  // so we "backpatch" the prev instruction
  patchJump(thenJump);
  emitByte(OP_POP);
